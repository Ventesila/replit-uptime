...
//=====// LinkEklemeFormu \\=====\\
const LinkEklemeFormu = new ModalBuilder()//Erasty
    .setCustomId('linkeklemeform2')
    .setTitle('Link ekle')
const LinkEkleFormu = new TextInputBuilder()//Erasty
    .setCustomId('linkekle')
    .setLabel('Proje adresinizi giriniz.')
    .setStyle(TextInputStyle.Paragraph)
    .setMinLength(20)
    .setMaxLength(100)//Erasty
...
const LinkSilFormu = new TextInputBuilder()
    .setCustomId('linksil')
    .setLabel('Proje adresinizi giriniz.')//Erasty
    .setStyle(TextInputStyle.Paragraph)
    .setMinLength(20)
    .setMaxLength(100)
    .setPlaceholder('https://erasty-uptime.glitch.me')
    .setRequired(true)//Erasty//Erasty
...
const SilID = new TextInputBuilder()//Erasty
    .setCustomId('silid')
    ...
    .setRequired(true)
const SilmeID = new ActionRowBuilder().addComponents(SilID);//Erasty
...
const Sebep = new TextInputBuilder()
    ...
    .setStyle(TextInputStyle.Paragraph)
    .setPlaceholder('Geçersiz link.')//Erasty
    .setRequired(true)
...
client.on('interactionCreate', async interaction => {//Erasty
    if (interaction.type !== InteractionType.ModalSubmit) return;
    if (interaction.customId === 'silmeform') {
      
      let linkInput = interaction.fields.getTextInputValue("sil")//Erasty
      ...
      if (!links.includes(linkInput)) return interaction.reply({embeds: [ProjeYok]}).catch(e => { })//Erasty
      ...
      //Erasty
        ...
        let PreVarmı;
        if(!PremiumVarmı) {
        PreVarmı = "<:No:1122993152064765973>"//Erasty
        } else {
        PreVarmı = "<:Yes:1122994864049619127>"
        }
      
      
        const ProjeSilindi = new EmbedBuilder()//Erasty
         .setColor("Red")
         ...
         .addFields({name: `<:Id:1122977905727778846> **Kullanıcı id**`, value: `${linkID}`})//Erasty
         .addFields({name: `<:Belge:1046825193131225169> **Sistemdeki link sayısı**`, value: `${db.fetch(`UptimeLink`).length}`})//Erasty
...
client.on('guildCreate', guild => {
  ...
  const Eklendim = new EmbedBuilder()
     ...
     .addFields({name: `<:Sunucu:1122988494680232077> **Toplam sunucu**`, value: `${client.guilds.cache.size}`})//Erasty
  ...
  client.on('guildDelete', guild => {//Erasty
    
    const Atıldım = new EmbedBuilder()
     .setColor("Red")
     .setTitle("Bir sunucudan atıldım")//Erasty
     .addFields({name: `<:Isim:1122992989325766748> **Sunucu adı**`, value: `${guild}`})
     .addFields({name: `<:Id:1122977905727778846> **Sunucu id**`, value: `${guild.id}`})
     .addFields({name: `<:Sunucu:1122988494680232077> **Toplam sunucu**`, value: `${client.guilds.cache.size}`})
     .addFields({name: `<:Kullanici:1122988494680232077> **Toplam kullanıcı**`, value: `${client.users.cache.size}`})
  client.channels.cache.get('1112800960570462268').send({embeds: [Atıldım]})})//Erasty
//=====// EklendimAtıldım \\=====\\
    ...
    if (interaction.customId === "eklebuton") {
      ...
      const Kullanamazsın = new EmbedBuilder()
        ...
        const Destek = new ActionRowBuilder().addComponents(new ButtonBuilder()        
           .setURL(`https://discord.gg/XjBRvvaUzM`)//Erasty
...
client.on('interactionCreate', async interaction => {
    if (interaction.type !== InteractionType.ModalSubmit) return;
    if (interaction.customId === 'linkeklemeform2') {//Erasty
