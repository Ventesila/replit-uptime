...
module.exports = {
    ...
    async execute(client, interaction) {   
      ...
      if(interaction.user.id !== "1112637700604960880" && interaction.user.id !== "788717157919096854" && interaction.user.id !== "SAHİB" && interaction.user.id !== "SAHİB" && interaction.user.id !== "1059475189588570122"){
      ...
      const PremiumEklendi = new EmbedBuilder()
         .setColor("Green")
         .setTitle("Başarılı")
         .setDescription(`<:Tik:1117512041419251752> ${kullanıcı} **adlı kullanıcıya premium verildi.**`)
        
      const PremiumVar = new EmbedBuilder()
         .setColor("Red")
         .setTitle("Hata")
         .setDescription(`<:Carpi:1117512106833612883> ${kullanıcı} **adlı kullanıcının zaten premium üyeliği bulunuyor.**`)
